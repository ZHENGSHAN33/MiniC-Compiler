...
1. 公共接口集中在 `include/common.hpp`，组员之间按 Token、AST、Symbol、IR 边界对接。
2. 主控程序已经串起完整流程，后续成员可以替换或增强单个模块，不需要重写框架。
3. IR 优化支持常量折叠、常量传播和简单不可达代码删除，可直接用 `--ir` 与 `--ir --opt` 对比。
4. VM 后端可以直接执行编译结果，答辩时能现场演示，不依赖复杂汇编环境。
5. AST 可输出 Graphviz DOT，方便后续生成图放入报告或 PPT。
6. `-S` 可以输出 x86 风格汇编，作为后端扩展和答辩亮点。
...
目前 `src/main.cpp` 是一个完整集成版，方便先跑通演示。后续如果成员要拆模块，可以按下面方向逐步拆，不要一次性大改：
