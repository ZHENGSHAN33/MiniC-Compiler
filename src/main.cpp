...
private:
    ...
    void synchronizeStmt() {
        while (!check(TokenKind::END_OF_FILE) && !check(TokenKind::RBRACE)) {
            ...
            if (check(TokenKind::KW_INT) || check(TokenKind::KW_BOOL) || check(TokenKind::KW_IF) ||
                check(TokenKind::KW_WHILE) || check(TokenKind::KW_RETURN)) return;
            ++pos_;
        }
    }

    void synchronizeTop() {
        while (!check(TokenKind::END_OF_FILE) && !check(TokenKind::KW_INT)) ++pos_;
    }
    ...
    std::unique_ptr<ASTNode> parseStmt() {
        try {
            if (check(TokenKind::KW_INT) || check(TokenKind::KW_BOOL)) return parseVarDecl();
            if (check(TokenKind::LBRACE)) return parseBlock();
            if (match(TokenKind::KW_IF)) return parseIf(previous().loc);
            if (match(TokenKind::KW_WHILE)) return parseWhile(previous().loc);
            ...
            if (match(TokenKind::KW_RETURN)) return parseReturn(previous().loc);
            if (match(TokenKind::KW_READ)) return parseRead(previous().loc);
            if (match(TokenKind::KW_WRITE)) return parseWrite(previous().loc);
    ...
    std::unique_ptr<ASTNode> parseIf(SourceLocation loc) {
        ...
        if (match(TokenKind::KW_ELSE)) node->children.push_back(parseStmt());
        return node;
    }
    ...
    std::unique_ptr<ASTNode> parseOr() {
        auto left = parseAnd();
        while (match(TokenKind::OR)) left = binary("||", std::move(left), parseAnd(), previous().loc);
        return left;
    }

    std::unique_ptr<ASTNode> parseAnd() {
        auto left = parseEquality();
        while (match(TokenKind::AND)) left = binary("&&", std::move(left), parseEquality(), previous().loc);
        return left;
    }
    ...
    std::unique_ptr<ASTNode> parsePrimary() {
        ...
        if (match(TokenKind::IDENT)) return makeNode(ASTKind::Identifier, previous().lexeme, previous().loc);
...
void printAST(std::ostream& out, const ASTNode* node, int depth = 0) {
    ...
    if (node->type != TypeKind::Error) out << " : " << typeName(node->type);
    out << "\n";
    for (const auto& child : node->children) printAST(out, child.get(), depth + 1);
}
